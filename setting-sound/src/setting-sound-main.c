 ...
 ************************************************/
static Eina_Bool
setting_sound_main_click_softkey_back_cb(void *data, Evas_Object *obj,
                                                   void *event_info)
{
	setting_retvm_if(data == NULL, EINA_FALSE, "Data parameter is NULL");
	SettingSoundUG *ad = (SettingSoundUG *) data;
	if (ad->empty_flag) {
		return EINA_FALSE;
	}
                                                                  ...
                                                                  Evas_Object *obj, void *event_info)
{
	...
	if (!safeStrCmp(KeyStr_VPN, list_item->keyStr)) {
 ...
 * @param event_info event type
 */
static void __setting_sound_main_vconf_changed_cb(keynode_t *key, void *data)
{
	SETTING_TRACE_BEGIN;
 ...
 **************************************************/
int setting_sound_main_generate_genlist(void *data)
{
	SETTING_TRACE_BEGIN;
	/* error check */
...
static int setting_sound_main_create(void *cb)
{
	...
	SettingSoundUG *ad = (SettingSoundUG *) cb;
	...
	ad->ly_main =
	    setting_create_layout_navi_bar_genlist(ad->win_main_layout,
	                                           ...
	                                           NULL,
	                                           (setting_call_back_func)setting_sound_main_click_softkey_back_cb,
	                                           NULL, ad, &scroller,
	                                           &(ad->navi_bar));
	ad->genlist = scroller;

	setting_sound_main_generate_genlist((void *)ad);

	int ret = vconf_notify_key_changed(VCONFKEY_LOCATION_USE_MY_LOCATION, __setting_sound_main_vconf_changed_cb, ad);
	if (ret != 0) {
	...
	setting_view_sound_main.is_create = 1;
	SETTING_TRACE_END;
	return SETTING_RETURN_SUCCESS;
}
...
static int setting_sound_main_destroy(void *cb)
{
	...
	int ret = vconf_ignore_key_changed(VCONFKEY_LOCATION_USE_MY_LOCATION, __setting_sound_main_vconf_changed_cb);
	if (ret != 0) {
	...
	setting_view_sound_main.is_create = 0;
